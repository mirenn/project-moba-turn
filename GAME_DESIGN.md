...
*   **行動解決のタイミング (Resolution Timing)**:
    ...
    *   **解決フェーズ開始時に、すべてのプレイヤーが選んだ行動（カード）が公開される。** これは公開情報であり、相手の選んだ行動を確認した上で移動先・攻撃対象を決定できる。
...
| カード名 | タイプ | 優先度 | 攻撃力 | 移動 | 範囲 | 効果 |
|----------|--------|--------|--------|------|------|------|
| とんぼがえり | むし | 100 | 20 | 1マス | 1 | 隣接1体に攻撃。成功時、控えと交代 |
| みずしゅりけん | みず | 90 | 20×2〜4 | 0マス | 1 | 隣接1体に20ダメージを2〜4回攻撃 |
| あくのはどう | あく | 75 | 80 | 0マス | 1 | 隣接1体に攻撃。20%で相手のこのターンの行動を無効化 |
| かげうち | あく | 120 | 30 | 3マス | 1 | 隣接1体に攻撃 |
...
| ふみつけ | じめん | 60 | 50 | 1マス | 1 | 隣接1体に攻撃。30%で相手を1マス押し出す |
| アイアンヘッド | はがね | 70 | 80 | 1マス | 1 | 隣接1体に攻撃。30%で相手をひるませる |
...
*   **属性**: ほのお (Fire)
*   **HP**: 85
*   **特性**: もうか（HPが1/3以下のとき、ほのお技の威力が1.5倍）

| カード名 | タイプ | 優先度 | 攻撃力 | 移動 | 範囲 | 効果 |
|----------|--------|--------|--------|------|------|------|
| かえんほうしゃ | ほのお | 65 | 90 | 0マス | 2 | 2マス以内の1体に攻撃 |
| フレアドライブ | ほのお | 80 | 120 | 2マス | 1 | 2マス移動後、隣接1体に攻撃。自分も1/3ダメージを受ける |
| ほのおのキバ | ほのお | 95 | 65 | 1マス | 1 | 隣接1体に攻撃。10%で相手をやけど状態にする |
| 交代 | - | 45 | - | 0マス | - | 控えのチャンピオンと交代する |
...
*   **属性**: でんき (Electric)
*   **HP**: 75
*   **特性**: せいでんき（接触攻撃を受けたとき、30%で相手をまひ状態にする）

| カード名 | タイプ | 優先度 | 攻撃力 | 移動 | 範囲 | 効果 |
|----------|--------|--------|--------|------|------|------|
| 10まんボルト | でんき | 70 | 90 | 0マス | 2 | 2マス以内の1体に攻撃。10%で相手をまひ状態にする |
| でんこうせっか | ノーマル | 115 | 40 | 2マス | 1 | 2マス移動後、隣接1体に攻撃 |
| ボルトチェンジ | でんき | 85 | 70 | 0マス | 2 | 2マス以内の1体に攻撃。成功時、控えと交代 |
| 交代 | - | 40 | - | 0マス | - | 控えのチャンピオンと交代する |
